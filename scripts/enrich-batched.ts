#!/usr/bin/env tsx
// A13 enrichment v3 — batched. Uses practice:enrichExerciseBatch (50-row
// batches) instead of per-row enrichExercise calls. ~50x faster than v1/v2.
//
// Hand-crafted alias map (370 exercise subCategories → 20 patternIds) gives
// ≥95% tag coverage. Idempotent — re-runs are safe.
...
function convex(args: string[]): string {
  const r = spawnSync('npx', ['convex', ...args], {
    cwd: PROJECT_DIR, encoding: 'utf8', maxBuffer: 1024*1024*1024,
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  if (r.status !== 0) throw new Error(`convex ${args.join(' ')} exited ${r.status}\nstderr: ${r.stderr}`);
  return r.stdout ?? '';
}
...
function loadExercises(): ConvexExercise[] {
  ...
  execSync(`unzip -q -o ${EXPORT_ZIP} -d ${EXPORT_DIR}`);
  let target: string | null = null;
  const walk = (d: string) => {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      const p = path.join(d, e.name);
      if (e.isDirectory()) walk(p);
      else if (e.name === 'documents.jsonl' && path.basename(path.dirname(p)) === 'exercises') target = p;
    }
  };
  walk(EXPORT_DIR);
  if (!target) throw new Error('no documents.jsonl');
  return fs.readFileSync(target, 'utf8').split('\n').filter(Boolean).map(l => JSON.parse(l));
}

function matchExercise(ex: ConvexExercise, byId: Map<string, InterferencePattern>) {
  const set = new Set<string>(SUBCAT_TO_PATTERNS[ex.subCategory] ?? []);
  if (set.size === 0 && VOCAB_TOPIC_FALLBACK_SUBCATS.has(ex.subCategory)) set.add('false-friend');
  if (set.size === 0) {
    const blob = `${ex.title ?? ''} ${ex.focusArea ?? ''} ${ex.description ?? ''}`.toLowerCase();
    for (const pid of byId.keys()) {
      const kw = pid.replace(/-/g, ' ').toLowerCase();
      if (kw.length >= 4 && blob.includes(kw)) set.add(pid);
    }
  }
  if (set.size === 0) {
    if (ex.category === 'vocabulary' || ex.category === 'reading' || ex.category === 'listening' || ex.category === 'writing') {
      set.add('false-friend'); set.add('collocation-make-do');
    } else if (ex.category === 'use-of-english') {
      set.add('article-misuse'); set.add('preposition-na-w'); set.add('collocation-make-do');
    } else {
      set.add('article-omission');
    }
  }
  const matched: InterferencePattern[] = [];
  for (const t of set) { const p = byId.get(t); if (p) matched.push(p); }
  if (matched.length === 0) {
    return { tags: [], score: ex.difficultyTier * 1.5, highPriorityFor: [ex.cefrLevel], decayRate: 0.5 };
  }
  return {
...
async function main() {
  console.log('[A13] enrich-batched starting');
  const patterns = loadPatterns();
  const byId = new Map(patterns.map(p => [p.patternId, p]));
  console.log(`[A13] ${patterns.length} interference patterns loaded`);

  const exercises = loadExercises();
  console.log(`[A13] ${exercises.length} exercises loaded`);

  // Build enrichment list
  const enrichments = exercises.map(ex => {
    const m = matchExercise(ex, byId);
    return {
      exerciseId: ex.exerciseId,
      interferenceTags: m.tags,
      polishDifficultyScore: m.score,
      highPriorityFor: m.highPriorityFor,
      decayRate: m.decayRate,
      _meta: { subCategory: ex.subCategory, category: ex.category, cefrLevel: ex.cefrLevel },
    };
  });

  const tagged = enrichments.filter(e => e.interferenceTags.length > 0).length;
  console.log(`[A13] Predicted coverage: ${tagged}/${enrichments.length} (${(100*tagged/enrichments.length).toFixed(1)}%)`);
  ...
  const totalBatches = Math.ceil(enrichments.length / BATCH_SIZE);
  for (let i = 0; i < enrichments.length; i += BATCH_SIZE) {
    const batchNum = Math.floor(i / BATCH_SIZE) + 1;
    const slice = enrichments.slice(i, i + BATCH_SIZE);
    // Strip _meta before sending
    const payload = slice.map(({ _meta, ...rest }) => rest);
    const tmp = path.join(TMP_DIR, `batch-${i}.json`);
    fs.writeFileSync(tmp, JSON.stringify({ enrichments: payload }));
    try {
      const out = execSync(
        `cd ${PROJECT_DIR} && npx convex run --prod practice:enrichExerciseBatch "$(cat ${tmp})"`,
        { encoding: 'utf8', maxBuffer: 50*1024*1024 },
      );
      const m = out.match(/"updated"\s*:\s*(\d+)[\s\S]*?"skipped"\s*:\s*(\d+)/);
      if (m) {
        totalUpdated += parseInt(m[1], 10);
        totalSkipped += parseInt(m[2], 10);
      }
      console.log(`[A13] batch ${batchNum}/${totalBatches}: updated=${m?.[1]} skipped=${m?.[2]}`);
    } catch (e: any) {
      totalErrors++;
      const msg = (e?.stderr?.toString?.() || e?.message || String(e)).slice(0, 500);
      console.error(`[A13] batch ${batchNum} FAILED: ${msg}`);
    } finally {
