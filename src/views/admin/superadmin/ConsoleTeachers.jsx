...
export default function ConsoleTeachers() {
  ...
  const { rows, error, reload } = useConvexList(
    () => listTeachers(schoolId, showRemoved), [schoolId, showRemoved], !!schoolId)
  ...
  if (!schoolId) {
    return (
      <div className="sa-page">
        <div className="sa-page-header"><h1 className="sa-page-title">Teachers</h1></div>
        <NeedSchool what="Teachers" />
      </div>
    )
  }
    ...
    <div className="sa-page">
      <div className="sa-page-header">
        <div>
          ...
          <p className="sa-page-sub">
            Teaching staff in <strong>{school?.name}</strong>, with how many students name each of
            them as primary teacher.
          </p>
        </div>
      ...
      {error ? <ConsoleErrorPanel error={error} onRetry={reload} />
        : rows === null ? <ConsoleSkeleton rows={4} label="Loading teachers…" />
          : !rows.length ? (
            <ConsoleEmpty icon="school" title="No teachers in this school yet"
                    ...
                    <td className="sa-row-actions">
                      {t.removed ? (
                        <button type="button" className="sa-btn sa-btn-ghost sa-btn-sm"
                                onClick={() => askRestore(t)}>Restore</button>
                      ) : (
                        <>
                          <button type="button" className="sa-btn sa-btn-ghost sa-btn-sm"
                                  onClick={() => { setNote(null); setDraft({ _id: t._id, name: t.name, email: t.email }) }}>
                            Edit
                          </button>
                          <button type="button" className="sa-btn sa-btn-ghost sa-btn-sm"
                                  onClick={() => askRemove(t)}>Remove</button>
                        </>
                      )}
                    </td>
                  </tr>
