...
export type TweakRadioOption<V extends string = string> = V | { value: V; label: React.ReactNode };

export interface TweakRadioProps<V extends string = string> {
  label: React.ReactNode;
  value: V;
...
export function TweakRadio<V extends string = string>({ label, value, options, onChange }: TweakRadioProps<V>) {
  const trackRef = useRef<HTMLDivElement | null>(null);
  const [dragging, setDragging] = useState(false);
  const opts = options.map((o) => (typeof o === 'object' ? o : { value: o, label: o })) as { value: V; label: React.ReactNode }[];
...
export type TweakSelectOption<V extends string = string> = V | { value: V; label: React.ReactNode };

export interface TweakSelectProps<V extends string = string> {
  label: React.ReactNode;
  value: V;
  options: TweakSelectOption<V>[];
  onChange: (v: V) => void;
}

export function TweakSelect<V extends string = string>({ label, value, options, onChange }: TweakSelectProps<V>) {
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e) => onChange(e.target.value as V)}>
        {options.map((o) => {
          ...
          return <option key={v} value={v}>{l as string}</option>;
        })}
      </select>
    </TweakRow>
  );
