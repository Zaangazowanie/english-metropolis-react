...
// ─────────────────────────────────────────────────────────────
// Host bridge — `window.omelette` is injected by the prototype host (and is
// undefined in standalone builds). Keep a tiny declaration so TypeScript can
// see the optional shape without leaking globals.
// ─────────────────────────────────────────────────────────────
declare global {
  interface Window {
    omelette?: {
...
interface DCContextValue {
  ...
  patchSection: (id: string, p: Partial<SectionState> | ((prev: SectionState) => Partial<SectionState>)) => void;
...
export function DesignCanvas({ children, minScale, maxScale, style }: DesignCanvasProps) {
  const [state, setState] = useState<DCState>({ sections: {}, focus: null });
  ...
  useEffect(() => {
    ...
    fetch('./' + DC_STATE_FILE)
      ...
      .catch(() => { /* sidecar absent — start fresh */ })
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      window.omelette?.writeFile(DC_STATE_FILE, JSON.stringify({ sections: state.sections })).catch(() => { /* host write failures are non-fatal */ });
    }, 250);
  ...
  const registry: Record<string, { sectionId: string; artboard: React.ReactElement<DCArtboardProps> }> = {};
  const sectionMeta: Record<string, { title: string; subtitle?: React.ReactNode; slotIds: string[] }> = {};
  const sectionOrder: string[] = [];
  React.Children.forEach(children, (sec) => {
    if (!React.isValidElement(sec) || sec.type !== DCSection) return;
    const secEl = sec as React.ReactElement<DCSectionProps>;
    const sid = secEl.props.id ?? secEl.props.title;
    if (!sid) return;
    sectionOrder.push(sid);
    const persisted = state.sections[sid] || {};
    const srcIds: string[] = [];
    React.Children.forEach(secEl.props.children, (ab) => {
      if (!React.isValidElement(ab) || ab.type !== DCArtboard) return;
      const abEl = ab as React.ReactElement<DCArtboardProps>;
      const aid = abEl.props.id ?? abEl.props.label;
      if (!aid) return;
      registry[`${sid}/${aid}`] = { sectionId: sid, artboard: abEl };
      srcIds.push(aid);
    });
    const kept = (persisted.order || []).filter((k) => srcIds.includes(k));
    sectionMeta[sid] = {
      title: persisted.title ?? secEl.props.title ?? sid,
      subtitle: secEl.props.subtitle,
...
// Safari-only gesture event shape (not in lib.dom.d.ts).
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }: DCViewportProps) {
  ...
  useEffect(() => {
    ...
    const onGestureStart = (ev: Event) => { const e = ev as GestureEvent; e.preventDefault(); isGesturing = true; gsBase = tf.current.scale; };
    const onGestureChange = (ev: Event) => {
      const e = ev as GestureEvent;
      e.preventDefault();
      zoomAt(e.clientX, e.clientY, (gsBase * e.scale) / tf.current.scale);
    };
    const onGestureEnd = (ev: Event) => { const e = ev as GestureEvent; e.preventDefault(); isGesturing = false; };
...
export function DCSection({ id, title, subtitle, children, gap = 48 }: DCSectionProps) {
  ...
  const artboards = all.filter((c): c is React.ReactElement<DCArtboardProps> =>
    React.isValidElement(c) && c.type === DCArtboard
  );
  const rest = all.filter((c) => !(React.isValidElement(c) && c.type === DCArtboard));
  const srcOrder = artboards.map((a) => (a.props.id ?? a.props.label) as string);
  ...
  const byId = Object.fromEntries(artboards.map((a) => [(a.props.id ?? a.props.label) as string, a]));
  ...
  return (
    <div data-dc-section={sid} style={{ marginBottom: 80, position: 'relative' }}>
      <div style={{ padding: '0 60px 56px' }}>
        <DCEditable tag="div" value={sec.title ?? title ?? sid}
      ...
      <div style={{ display: 'flex', gap, padding: '0 60px', alignItems: 'flex-start', width: 'max-content' }}>
        {order.map((k) => (
          <DCArtboardFrame key={k} sectionId={sid} artboard={byId[k]} order={order}
            label={(sec.labels || {})[k] ?? byId[k].props.label ?? k}
...
export function DCArtboard(_props: DCArtboardProps): null { return null; }

interface DCArtboardFrameProps {
  sectionId: string;
  artboard: React.ReactElement<DCArtboardProps>;
  label: string;
  order: string[];
...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus }: DCArtboardFrameProps) {
  ...
  const id = (rawId ?? rawLabel) as string;
...
interface DCFocusOverlayProps {
  entry: { sectionId: string; artboard: React.ReactElement<DCArtboardProps> };
  sectionMeta: Record<string, { title: string; subtitle?: React.ReactNode; slotIds: string[] }>;
  sectionOrder: string[];
}
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }: DCFocusOverlayProps) {
  ...
  const aid = (artboard.props.id ?? artboard.props.label) as string;
