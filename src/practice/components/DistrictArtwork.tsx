...
export function DistrictArtwork({ shell, accent = '#00d9ff', index = 0 }: { shell: string; accent?: string; index?: number }) {
  const id = useId().replace(/:/g, '');
  return <svg className="em-district-art" viewBox="0 0 320 192" aria-hidden="true" focusable="false" style={{ color: accent }}>
    <defs>
      <radialGradient id={`${id}-halo`}><stop stopColor={accent} stopOpacity=".52"/><stop offset="1" stopColor={accent} stopOpacity="0"/></radialGradient>
      <linearGradient id={`${id}-body`} x2="1" y2="1"><stop stopColor={accent}/><stop offset=".4" stopColor="#1839a5"/><stop offset="1" stopColor="#090e34"/></linearGradient>
      <linearGradient id={`${id}-glass`} x2="1" y2="1"><stop stopColor="#5deaff" stopOpacity=".27"/><stop offset=".48" stopColor="#009fff" stopOpacity="0"/></linearGradient>
    </defs>
    <ellipse cx="164" cy="105" rx="145" ry="96" fill={`url(#${id}-halo)`}/>
    <g opacity=".65" stroke={accent} strokeWidth=".6">
    ...
    <path d="m34 159 123-28 131 27-123 31Z" fill="#11296b" stroke="#256aff" strokeWidth=".6"/>
    {[0,1,2,3,4].map(i=><path key={`pave-${i}`} d={`M${57+i*23} ${154-i*5}l118 29 M${64+i*23} ${166+i*5}l115-26`} stroke="#2372d4" strokeWidth=".55" opacity=".6"/>)}
    ...
    <g transform="translate(157 81)">
      ...
      <path d="m51-57 16 12-1 102-15 25V39l-8-10 8-61Z" fill="#071450"/>
      <path d="m58-34 4 3-1 58-5 10V5l-5 10Z" fill={accent}/>
      <path d="M-54-57H50M-54-34V65M66-40V55" stroke={accent} strokeWidth="2"/>
      ...
      <path d="M-48-32H43L37 29H-45Z" fill={`url(#${id}-glass)`}/>
      <path d="m-45 33 82 0 14 15-101-1Z" fill="#234ac4" stroke="#55dfff" strokeWidth=".6"/>
      ...
      <ellipse cx="15" cy="40" rx="5" ry="3" fill="#ffce00"/><ellipse cx="29" cy="40" rx="5" ry="3" fill="#ff268d"/>
      {[-42,40].map(x=><g key={`bolt-${x}`}><circle cx={x} cy="-46" r="1.5" fill="#e6faff"/><circle cx={x} cy="44" r="1.5" fill="#e6faff"/></g>)}
      <path d="M-43 52 H39 V70 L-43 59Z" fill="#151626"/>
      <path d="m-33 54 58 7" stroke={accent} strokeWidth="2" opacity=".65"/>
      <rect x="-7" y="61" width="11" height="3" rx="1" fill="#ffce00"/>
      {[0,1,2,3].map(i=><path key={`vent-${i}`} d={`M21 ${57+i*2}l13 1`} stroke="#45a8ee" strokeWidth="1"/>)}
    </g>
...
export function ArcadeCity() {
  ...
  return <svg className="em-arcade-city" viewBox="0 0 1000 310" preserveAspectRatio="xMidYMax slice" aria-hidden="true">
    <defs><linearGradient id={`${id}-tower`} x2="0" y2="1"><stop stopColor="#295aff"/><stop offset=".5" stopColor="#3c1faf"/><stop offset="1" stopColor="#07193f"/></linearGradient></defs>
    {[0,1].map(row=><g key={row} opacity={row ? .95 : .4}>{Array.from({length:24},(_,i)=>{const x=i*46-30+row*12,h=40+(i*47+row*29)%143,y=255-row*9;return <g key={i}>
      ...
      <path d={`M${x} ${y-h}l27-5 11 7-27 4Z`} fill={['#157bff','#7c23f5','#00a6b6'][i%3]}/>
