...
export function ArcadeCabinet({ title, accent, number, shellId, children }: Props) {
  ...
  return <section ref={cabinet} className="em-arcade-cabinet" data-run-state={run.last ?? 'ready'} style={{ '--arcade-accent': accent } as React.CSSProperties} aria-label={`${title} arcade`}>
    ...
    <div className="em-arcade-screen"><ArcadeEventsContext.Provider value={emit}><ArcadeFeedbackContext.Provider value={report}>{children}</ArcadeFeedbackContext.Provider></ArcadeEventsContext.Provider></div>
