...
<div className="action-three-vault-slot"><ActionPlayfield3D kind="openthebox" onShortcut={key=>{if(forcedState||completed||lockBusy||activeIdx===null||curBox?.face!=='open'||!cur)return false;const option=key.length===1?key.charCodeAt(0)-97:-1;if(option>=0&&option<cur.options.length){setDial(option);return true;}if(key==='arrowleft'||key==='arrowright'){setDial(i=>(i+(key==='arrowright'?1:-1)+cur.options.length)%cur.options.length);return true;}if(key==='enter'){pick(activeIdx,dial);return true;}return false;}} data={{reducedMotion:reduceMotion,dial,onPick:openBox,onDial:()=>{if(cur)setDial(d=>(d+1)%cur.options.length);},actors:boxes.map((b,i)=>({id:i,x:0,y:0,state:b.face,selected:activeIdx===i,enabled:b.face!=='sealed'&&(activeIdx===null||activeIdx===i)}))}} controls={<>{boxes.map((b,i)=><button key={i} disabled={b.face==='sealed'||(activeIdx!==null&&activeIdx!==i)} onClick={()=>openBox(i)}><kbd>{i+1}</kbd>Safe {i+1}{b.face==='sealed'?' ✓':''}</button>)}</>} /></div>
          </div>
