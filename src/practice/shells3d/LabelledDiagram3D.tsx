...
const design = { kind: 'patch', title: 'The City Patchbay', instruction: 'Select a label plug, then connect it to the numbered socket from the diagram. Incorrect plugs return to the tray.', action: 'Connect label', color: '#97d2bd', mode: 'assembly' } as const;
...
export default function LabelledDiagram3D(props: MachineProps) {
  ...
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={puzzle} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'labelleddiagram'}) : undefined} /></div></Challenge3DPreferences>;
