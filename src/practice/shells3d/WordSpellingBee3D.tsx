...
export default function WordSpellingBee3D({length,draft,done,onChange,onHear,onSubmit}:{length:number;draft:string;done:boolean;onChange:(text:string)=>void;onHear:()=>void;onSubmit:()=>void}){
 ...
 return <Stage accent="#ffcf00" secondary="#c124ff" title="The spelling sound lock" instruction="Hear the word, select a tumbler, and turn its letter wheel. Unlock when the spelling is ready." width={Math.max(11,cols*1.1+2)} height={Math.max(8,rows*1.5+5)}>
