...
const design = { kind: 'sentence', title: 'The Sentence Express', instruction: 'Couple the word wagons in the right sequence. Launch the train only when the whole sentence is assembled.', action: 'Launch sentence', color: '#9ebbdc', mode: 'assembly' } as const;
...
export default function Unjumble3D(props: MachineProps) {
  ...
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={puzzle} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'unjumble'}) : undefined} /></div></Challenge3DPreferences>;
