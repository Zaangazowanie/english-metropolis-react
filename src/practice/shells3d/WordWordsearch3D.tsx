import {CityBuildings} from './word-kit/Machines';
...
export default function WordWordsearch3D({grid,routes,onTrail}:{grid:string[][];routes:{start:Cell;end:Cell}[];onTrail:(start:Cell,end:Cell)=>void}){
 ...
 return <><Stage navigate title="Rooftop word trails" instruction="Tap the first rooftop and the last rooftop in one straight word. Correct routes light up the city." width={size*.78+2} height={size*.78+.8}>
  <CityBuildings size={size} neon={true}/><Grid rows={size} cols={size} cells={grid.flatMap((row,r)=>row.map((label,c)=>({r,c,label,active:!!anchor&&r===anchor[0]&&c===anchor[1],done:routes.some(route=>pointOnTrail([r,c],route.start,route.end))})))} onPick={pick}/>
  {routes.map((route,i)=><Rail key={i} from={pos(route.start)} to={pos(route.end)} color="#77e8bb" thick={.12}/>)}{anchor&&cursor&&<Rail from={pos(anchor)} to={pos(cursor)} color="#ffd582" thick={.1}/>}
