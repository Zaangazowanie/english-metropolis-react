...
const design = { kind: 'dealer', title: 'The Midnight Card Table', instruction: 'Deal a physical challenge card. Choose an answer terminal and bank the card to commit.', action: 'Bank this card', color: '#d0a6ba', mode: 'choice' } as const;
...
export default function RandomCards3D(props: MachineProps) {
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={props.puzzle as never} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'randomcards'}) : undefined} /></div></Challenge3DPreferences>;
