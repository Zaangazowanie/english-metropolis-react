...
export default function WordOpenCloze3D({gaps,active,onSelect,onSeal}:{gaps:{id:number;value:string;done:boolean;wrong:boolean;skipped:boolean}[];active:number;onSelect:(id:number)=>void;onSeal:(id:number)=>void}){
 ...
 return <Stage accent="#00d7ff" secondary="#9127ff" title="The missing-word bridge" instruction="Select a sunken bridge span, write its missing word below, then pull the seal lever." width={Math.max(10,cols*1.9+1)} height={8}>
