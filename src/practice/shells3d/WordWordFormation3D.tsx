...
export default function WordWordFormation3D({base,draft,done,onBuild,onSubmit}:{base:string;draft:string;done:boolean;onBuild:(word:string)=>void;onSubmit:()=>void}){
 ...
 return <Stage accent="#00e1ff" secondary="#ff7012" title="Morphology assembly press" instruction="Snap a prefix and suffix around the root. Refine the spelling below, then press the new form." width={12} height={11}>
  ...
  <Piece position={[0,.4,.2]} width={3.2} color="#ff9700" label={base} onPick={()=>build('','')}/>
  {prefixes.map((p,i)=><Piece key={p} position={prefix===p?[-3,.4,.2]:[-4.5+i*2.8,2.5,0]} width={1.8} color="#ff278e" label={p+'-'} active={prefix===p} onPick={()=>build(prefix===p?'':p,suffix)}/>)}
  {suffixes.map((s,i)=><Piece key={s} position={suffix===s?[3,.4,.2]:[(i%3-1)*3,-1.8-Math.floor(i/3)*1.3,0]} width={1.8} label={'-'+s} active={suffix===s} onPick={()=>build(prefix,suffix===s?'':s)} color="#00beef"/>)}
