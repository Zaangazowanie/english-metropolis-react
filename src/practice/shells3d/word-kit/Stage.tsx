...
import type { CSSProperties, ReactNode, RefObject, KeyboardEvent } from 'react';
...
export function Piece({position,label,onPick,active=false,done=false,wrong=false,shape='block',color='#762aff',width=1.2,scale=1,rotate=0,disabled=false}: {position:Point;label?:string;onPick?:()=>void;active?:boolean;done?:boolean;wrong?:boolean;shape?:'block'|'train'|'crate'|'book'|'lamp'|'lock'|'lever';color?:string;width?:number;scale?:number;rotate?:number;disabled?:boolean}) {
 ...
 const [hovered,setHovered]=useState(false);const [focused,setFocused]=useState(false);const material=useRef<MeshStandardMaterial>(null);
 const lit=!disabled&&!!onPick&&(hovered||focused);
 useFrame((_,dt)=>{if(material.current)material.current.emissiveIntensity+=((active?.55:done?.4:lit?.62:.14)-material.current.emissiveIntensity)*motionFraction(dt,reducedMotion);});
 ...
 return <group ref={ref} position={initialPosition.current} scale={scale} onPointerOver={e=>{e.stopPropagation();setHovered(true);}} onPointerOut={()=>setHovered(false)} onClick={e=>{e.stopPropagation();pick();}}>
   <mesh><boxGeometry args={[width,.72,shape==='book'?.2:.5]}/><meshStandardMaterial ref={material} color={shade} roughness={.2} metalness={.38} emissive={shade} emissiveIntensity={active?.42:done?.34:.16}/></mesh>
   <mesh position={[0,-.29,.27]}><boxGeometry args={[width*.87,.035,.025]}/><meshStandardMaterial color={lit?'#fff4a2':'#77eaff'} emissive={lit?'#ffda00':'#00adff'} emissiveIntensity={lit?1:.45} toneMapped={false}/></mesh>
   ...
   {label&&<Html portal={portal} center position={[0,0,.31]} zIndexRange={[4,0]}><button ref={labelRef} type="button" title={label} aria-label={label} className={`word3d-label ${label.length<3?'is-letter':''} ${active?'is-active':''}`} data-hovered={lit} disabled={disabled||!onPick} onPointerEnter={()=>setHovered(true)} onPointerLeave={()=>setHovered(false)} onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)} onClick={e=>{e.stopPropagation();pick();}} aria-pressed={active} style={{width:label.length<3?40:Math.max(70,Math.min(140,width*65))}}>{label}</button></Html>}
 </group>;
}
...
export function Grid({rows,cols,cells,onPick,onHover,onCancel}: {rows:number;cols:number;cells:Array<{r:number;c:number;label:string;active?:boolean;done?:boolean;wrong?:boolean;hint?:boolean;ariaLabel?:string}>;onPick:(r:number,c:number,source?:'keyboard'|'pointer')=>void;onHover?:(r:number,c:number)=>void;onCancel?:()=>void}) {
 const portal=useContext(WordLabelPortal);
 const [hovered,setHovered]=useState<string|null>(null);const [focused,setFocused]=useState<string|null>(null);const [tabCell,setTabCell]=useState('0,0');const buttons=useRef(new Map<string,HTMLButtonElement>());
 const ref=useRef<InstancedMesh>(null);const scratch=useMemo(()=>({object:new Object3D(),color:new Color()}),[]);const {size}=useThree();
 useLayoutEffect(()=>{if(!ref.current)return;cells.forEach((cell,i)=>{const hot=hovered===`${cell.r},${cell.c}`||focused===`${cell.r},${cell.c}`;scratch.object.position.set((cell.c-(cols-1)/2)*.78,((rows-1)/2-cell.r)*.78,cell.done?.32:cell.active?.19:hot?.12:0);scratch.object.scale.set(.69,.69,cell.done?1:cell.active?.72:hot?.57:.34);scratch.object.updateMatrix();ref.current!.setMatrixAt(i,scratch.object.matrix);scratch.color.set(cell.wrong?'#ff2355':cell.done?'#00ca89':cell.active?'#ffbd00':cell.hint?'#ffef62':hot?'#00baff':(cell.r+cell.c)%3===0?'#7130ff':'#2458ee');ref.current!.setColorAt(i,scratch.color);});ref.current.instanceMatrix.needsUpdate=true;if(ref.current.instanceColor)ref.current.instanceColor.needsUpdate=true;},[cells,cols,rows,scratch,hovered,focused]);
 const entryCell=cells.some(cell=>`${cell.r},${cell.c}`===tabCell)?tabCell:cells[0]?`${cells[0].r},${cells[0].c}`:'';
 const keyDown=(e:KeyboardEvent<HTMLButtonElement>,r:number,c:number)=>{
  if(e.altKey||e.ctrlKey||e.metaKey)return;
  if(e.key==='Escape'&&onCancel){e.preventDefault();e.stopPropagation();onCancel();return;}
  const delta:Record<string,[number,number]>={ArrowLeft:[0,-1],ArrowRight:[0,1],ArrowUp:[-1,0],ArrowDown:[1,0]};
  if(!delta[e.key])return;e.preventDefault();e.stopPropagation();const [dr,dc]=delta[e.key];
  for(let nr=r+dr,nc=c+dc;nr>=0&&nr<rows&&nc>=0&&nc<cols;nr+=dr,nc+=dc){const button=buttons.current.get(`${nr},${nc}`);if(button){button.focus();break;}}
 };
 const labelSize=Math.max(18,Math.min(32,size.width/(cols+1)));
 return <><instancedMesh ref={ref} args={[undefined,undefined,cells.length]} onPointerMove={e=>{const cell=cells[e.instanceId??-1];if(cell){setHovered(`${cell.r},${cell.c}`);onHover?.(cell.r,cell.c);}}} onPointerOut={()=>setHovered(null)} onClick={e=>{e.stopPropagation();const cell=cells[e.instanceId??-1];if(cell)onPick(cell.r,cell.c);}}><boxGeometry/><meshStandardMaterial metalness={.35} roughness={.2} emissive="#2052ff" emissiveIntensity={.16}/></instancedMesh>{cells.map(cell=>{const id=`${cell.r},${cell.c}`;return <Html portal={portal} key={id} center position={[(cell.c-(cols-1)/2)*.78,((rows-1)/2-cell.r)*.78,cell.done?.88:cell.active?.62:.25]} zIndexRange={[4,0]}><button ref={node=>{if(node)buttons.current.set(id,node);else buttons.current.delete(id);}} type="button" data-grid-row={cell.r} data-grid-col={cell.c} data-hint={cell.hint||undefined} data-done={cell.done||undefined} tabIndex={entryCell===id?0:-1} className={`word3d-grid-letter ${cell.active?'is-active':''}`} style={{width:labelSize,height:labelSize,fontSize:Math.max(11,labelSize*.63)}} onPointerEnter={()=>{setHovered(id);onHover?.(cell.r,cell.c);}} onPointerLeave={()=>setHovered(null)} onFocus={()=>{setFocused(id);setTabCell(id);onHover?.(cell.r,cell.c);}} onBlur={()=>setFocused(null)} onKeyDown={e=>keyDown(e,cell.r,cell.c)} onClick={e=>onPick(cell.r,cell.c,e.detail===0?'keyboard':'pointer')} aria-pressed={!!cell.active} aria-label={cell.ariaLabel??`Row ${cell.r+1}, column ${cell.c+1}: ${cell.label||'empty'}`}>{cell.label||'·'}</button></Html>})}</>;
}
