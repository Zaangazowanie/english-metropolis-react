...
export function ForgePress({load,commit=0}:{load:number;commit?:number}){
 ...
 return <group>
  ...
  <Block at={[0,3.9,-.45]} size={[10,.65,1]} color="#e8340a"/><Rivets width={9} y={3.9} z={.1}/>
...
export function LiftTower({progress,lives}:{progress:number;lives:number}){
 ...
 return <group position={[-4.2,0,-.15]}>
  ...
  <Block at={[0,4,-.3]} size={[2.3,.38,.8]} color={BRASS}/><Wheel at={[0,3.87,.15]} r={.45}/><Block at={[0,0,.1]} size={[.035,7.8,.035]} color="#20daff"/>
...
export function VaultCabinet({open,route}:{open:boolean;route:'known'|'review'|null}){
 ...
 return <group>
  <Block at={[0,.75,-.9]} size={[5,3.55,.9]} color="#6020e4"/><Block at={[0,.75,-.38]} size={[4.38,2.88,.16]} color="#08081d"/>
...
export function Conveyor({destinations,selected}:{destinations:number;selected:boolean}){
 ...
 return <group><Block at={[0,-2.2,-.7]} size={[9,2.8,.35]} color="#080d27"/>{[-1,1].map(s=><Block key={s} at={[s*4.65,-2.2,-.33]} size={[.22,3.15,.3]} color={BRASS}/>)}
...
export function RailPlatforms({count=3}:{count?:number}){
 return <group>{[-1,1].map(side=><group key={side} position={[side*3.5,0,-.7]}><Block at={[0,0,0]} size={[3.1,count*1.65+1,.4]} color="#182c76"/><Block at={[-side*1.5,0,.26]} size={[.11,count*1.65+1,.08]} color={BRASS}/>{Array.from({length:count+1},(_,i)=><Block key={i} at={[0,(count/2-i)*1.65,.26]} size={[3,.045,.04]} color="#176aff"/>)}</group>)}
...
export function BridgeStructure({count,completed}:{count:number;completed:number}){
 return <group>{[-4.5,4.5].map(x=><group key={x}><Block at={[x,1,-.75]} size={[.3,5,.5]} color="#8221df"/><Block at={[x,3.55,-.65]} size={[.75,.35,.8]} color={BRASS}/></group>)}
...
export function SoundLock({slots,open}:{slots:number;open:boolean}){
 return <group><Block at={[0,.5,-.65]} size={[Math.min(10,slots*1.1+1.5),3.7,.75]} color="#a61adb"/><Block at={[0,.5,-.2]} size={[Math.min(9.6,slots*1.1+1.1),3.25,.13]} color="#151044"/>
...
export function PrintingPress({rows,pressed}:{rows:number;pressed:boolean}){
 return <group><Block at={[0,.5,-.7]} size={[12,Math.max(4,rows*1.4+1),.55]} color="#1249c7"/>{[-5.5,5.5].map(x=><group key={x}><Block at={[x,.5,-.24]} size={[.3,Math.max(4,rows*1.4+1),.3]} color={BRASS}/><Wheel at={[x,-rows*.7-1,.2]} r={.48}/></group>)}<Block at={[0,pressed?-.3:rows*.7+1,-.15]} size={[10.5,.32,.5]} color="#00c6ff"/><Rivets width={10} y={rows*.7+1} z={.13}/></group>;
}
...
export function TelegraphMachine({progress,ready,done}:{progress:number;ready:boolean;done:boolean}){
 ...
 return <group><Block at={[0,-2.7,-.7]} size={[11,1.8,.75]} color="#b83211"/><Block at={[0,-2.7,-.25]} size={[10.3,1.4,.18]} color="#381469"/>
