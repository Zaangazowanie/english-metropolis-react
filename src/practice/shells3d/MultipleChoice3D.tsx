...
const design = { kind: 'target', title: 'The Answer Vault', instruction: 'Aim at the terminal carrying the correct answer, then fire the key beam to unlock it.', action: 'Fire key beam', color: '#bc24ff', mode: 'choice' } as const;
...
export default function MultipleChoice3D(props: MachineProps) {
  ...
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#090d38"}:undefined}><CanonicalShell puzzle={puzzle} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'multiplechoice'}) : undefined} /></div></Challenge3DPreferences>;
