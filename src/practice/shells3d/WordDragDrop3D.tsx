...
import {CargoCrane,DistrictDetails} from './word-kit/Machines';
...
export default function WordDragDrop3D({pool,filled,count,selected,onSelect,onPlace,onRemove}:{pool:string[];filled:Record<number,string>;count:number;selected:string|null;onSelect:(word:string)=>void;onPlace:(gap:number,word:string)=>void;onRemove:(gap:number)=>void}){
 ...
 return <Stage accent="#00cfff" secondary="#ff6d00" title="Sentence cargo dock" instruction="Read the sentence below. Tab to a word crate and press Enter, then Tab to its numbered bay and press Enter. Select a loaded crate to unload it." width={Math.max(10,cols*2.4+2)} height={9}>
  <DistrictDetails kind="cargo" accent="#00cfff"/>
  <CargoCrane target={delivery?.gap??null} loaded={Object.keys(filled).length} count={count}/>
 {Array.from({length:count},(_,i)=><group key={i}><Rail from={[x(i),1.4-Math.floor(i/cols)*1.2,-.4]} to={[x(i),3.2,-.4]} color="#00daff"/><Piece position={[x(i),1.6-Math.floor(i/cols)*1.2,filled[i]?.35:0]} width={1.9} shape={filled[i]?'crate':'train'} label={filled[i]||`Bay ${i+1}`} done={!!filled[i]} active={!filled[i]&&!!selected} onPick={()=>filled[i]?onRemove(i):route(i)} disabled={flight.busy||(!filled[i]&&!selected)}/></group>)}
 {available.slice(visiblePage*12,visiblePage*12+12).map((word,i)=><Piece key={word+i} shape="crate" position={delivery?.word===word?[x(delivery.gap),1.6-Math.floor(delivery.gap/cols)*1.2,1.1]:[(i%4-1.5)*2.35,-1.1-Math.floor(i/4)*1.18,selected===word?1:0]} width={1.7} label={word} active={selected===word} color="#126dff" onPick={()=>onSelect(word)} disabled={flight.busy}/>)}
