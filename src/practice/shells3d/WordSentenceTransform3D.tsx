...
export default function WordSentenceTransform3D({keyword,ready,words,done,onKeyword,onSubmit,onNext}:{keyword:string;ready:boolean;words:number;done:boolean;onKeyword:()=>void;onSubmit:()=>void;onNext:()=>void}){
 return <Stage accent="#a123ff" secondary="#00e8ff" title="The sentence transfer station" instruction="Load the key-word capsule into your rewrite, then operate the transfer lever to send it through." width={12} height={8}>
  ...
  <Piece shape="train" width={2.5} position={[done?3.4:-3.4,-1.5,.25]} label={`${words} words`} color="#00bfff" done={done}/><Piece shape="lever" width={2.7} position={[3.4,1.1,.25]} label={done?'Next transfer':'Transfer rewrite'} active={ready&&!done} onPick={done?onNext:onSubmit}/>
 </Stage>;
}