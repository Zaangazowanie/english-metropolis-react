...
const design = { kind: 'memory', title: 'The Memory Vault', instruction: 'Open two numbered vault panels. A matching clue and word powers their memory circuit.', action: 'Open panel', color: '#a1c9d8', mode: 'direct' } as const;
...
export default function Concentration3D(props: MachineProps) {
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={props.puzzle as never} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'concentration'}) : undefined} /></div></Challenge3DPreferences>;
