...
const design = { kind: 'network', title: 'The Delivery Network', instruction: 'Connect a clue depot to its matching word depot. Completed connections stay illuminated.', action: 'Connect depot', color: '#aec995', mode: 'direct' } as const;
...
export default function FindTheMatch3D(props: MachineProps) {
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={props.puzzle as never} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'findthematch'}) : undefined} /></div></Challenge3DPreferences>;
