...
const design = { kind: 'crane', title: 'The Word Crane', instruction: 'Lift a word crate from the upper gantry, then install it in a numbered sign socket. Filled sockets can be cleared and rebuilt.', action: 'Install word', color: '#e0bd78', mode: 'assembly' } as const;
...
export default function GapFill3D(props: MachineProps) {
  ...
  if (props.items === undefined) return <Challenge3DPreferences value={{quality:props.quality,reducedMotion:props.reducedMotion}}><div style={props.fullscreen?{position:"fixed",inset:0,zIndex:900,overflow:"auto",background:"#14222d"}:undefined}><CanonicalShell puzzle={puzzle} onSessionComplete={props.onSessionComplete ? r => props.onSessionComplete?.({...r,shellKey:'gapfill'}) : undefined} /></div></Challenge3DPreferences>;
