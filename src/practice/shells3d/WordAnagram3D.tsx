...
export default function WordAnagram3D({tiles,slots,onPlace,onRemove,onCommit,done}:{tiles:{id:number;letter:string}[];slots:number[];onPlace:(id:number)=>void;onRemove:(i:number)=>void;onCommit:()=>void;done:boolean}){
 ...
 return <Stage title="Letter foundry" instruction="Lift an ingot, then tap the next forge socket. Tap a forged letter to remove it." width={Math.max(12,span+2)} height={Math.max(10,rows*2.3+5)}>
  <ForgePress load={slots.length/Math.max(1,tiles.length)} commit={press.tick}/>
  <Rail from={[-span/2,1,-.5]} to={[span/2,1,-.5]} color="#ffc773" thick={.2}/>
  {Array.from({length:tiles.length},(_,i)=><Piece key={`socket${i}`} position={[(i%cols-(cols-1)/2)*1.15,1.6-Math.floor(i/cols)*1.15,-.2]} width={.92} color="#526279" label={slots[i]===undefined?String(i+1):undefined} active={i===slots.length&&held!==null} onPick={()=>{if(held!==null&&i===slots.length&&!done){onPlace(held);setHeld(null);}}}/>)}
  {tiles.map((t,i)=>{const at=slots.indexOf(t.id);const selected=held===t.id&&at<0;return <Piece key={t.id} position={at>=0?[(at%cols-(cols-1)/2)*1.15,1.6-Math.floor(at/cols)*1.15,.3]:[(i%cols-(cols-1)/2)*1.15,poolY-Math.floor(i/cols)*1.1,selected?1.4:.2]} width={.85} label={t.letter} active={selected} done={done} color="#e5b786" onPick={()=>at>=0?onRemove(at):setHeld(t.id)} disabled={done||press.busy}/>;})}
