...
function StickyHeader() {
  ...
  const dd = open && (
    <div ref={ddRef}
      ...
      role="listbox" aria-label="Choose TTS voice">
      ...
      {QUICK.map(v => (
        <button key={v.id} type="button"
          ...
          <span className="text-lg" aria-hidden="true">{v.flag}</span>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate">{v.label}</p>
            <p className="text-[11px] text-slate-400 font-mono">{v.id}</p>
          </div>
          {voice === v.id && <span className="material-symbols-outlined text-[#0052d0] text-lg" aria-hidden="true">check</span>}
        </button>
      ))}
    </div>
  );
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="relative shrink-0" id="voiceSelector">
            <button ref={btnRef} id="voiceSelectorBtn"
              ...
              type="button" aria-haspopup="listbox" aria-expanded={open} aria-label="Text-to-speech voice selector" onClick={() => setOpen(p => !p)}>
              <span className="material-symbols-outlined text-slate-500 text-lg" aria-hidden="true">settings_voice</span>
              <span id="currentVoiceLabel" className="font-label text-sm text-slate-700">{cur.flag} {cur.label}</span>
              <span className={`material-symbols-outlined text-slate-400 text-sm transition-transform ${open ? 'rotate-180' : ''}`} aria-hidden="true">expand_more</span>
            </button>
            {dd}
          </div>
        </div>
        <nav aria-label="Primary" className="-mx-1 overflow-x-auto scrollbar-hide">
