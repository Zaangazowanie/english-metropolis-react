#!/bin/bash
# Plain feature previews and discounted package analysis; Convex before publication.
set -euo pipefail
umask 022
REPO=/root/englishmetro
WEB=/var/www/englishmetro
BASE=a513e4a939ead376d4ab14456289a28b9610e5ee
STAMP=$(date -u +%Y%m%d-%H%M%S)
BACKUP=/root/backups/englishmetro-ai-packages-$STAMP
...
git diff --quiet "$BASE" HEAD -- package.json package-lock.json index.html public/assets src/design src/views/v3/game-home.css src/views/v3/BajlaWalkthrough.jsx src/views/v3/bajla-tour.mjs
...
# Capture the live API, run offline checks and build before deploying the backend.
...
test "$(df --output=avail -B1 / | tail -1 | tr -d ' ')" -ge 21474836480
# No schema migration. Keep the previous source revision and live function list.
# Existing registered payments retain their stored amounts and completion path.
...
python3 - "$BACKUP/spec-before.json" "$BACKUP/function-spec.json" <<'SPEC'
import json,sys
def names(path):
    data=json.load(open(path))
    return {f.get('identifier') or f"HTTP {f['method']} {f['path']}" for f in (data if isinstance(data,list) else data['functions'])}
before,after=map(names,sys.argv[1:])
assert before == after, f"Unexpected API change: removed={before-after}, added={after-before}"
print(f"Backend deployed; all {len(after)} API entries preserved")
SPEC
...
echo "Homepage copy and AI package pricing published at $REV. Evidence: $BACKUP"
