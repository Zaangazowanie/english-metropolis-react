#!/bin/bash
# Availability range release: preserve the recurring schedule and existing bookings.
set -euo pipefail
umask 022
...
BACKUP=/root/backups/englishmetro-availability-range-$STAMP
. "$(dirname "$0")/_guard.sh"
guard_clean_prod "$REPO"
cd "$REPO"
exec 9>/root/englishmetro-availability-range-deploy.lock
flock -n 9
...
import sys
allowed={"src/views/admin/Calendar.jsx", "src/views/v3/LessonBooking.jsx", "deploy/deploy-availability-range-2026-09-07.sh"}
...
PY
trap - ERR
echo "Availability range published at $REV. Rollback and verification evidence: $BACKUP"
