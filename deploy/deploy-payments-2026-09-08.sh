#!/bin/bash
# Cards, wallets and guarded Raty readiness.
set -euo pipefail
umask 022
REPO=/root/englishmetro
WEB=/var/www/englishmetro
STAMP=$(date -u +%Y%m%d-%H%M%S)
BACKUP=/root/backups/englishmetro-payments-$STAMP
. "$(dirname "$0")/_guard.sh"
guard_clean_prod "$REPO"
cd "$REPO"
install -d -m 700 "$BACKUP"
node tests/p24-pipeline.test.mjs
bash tests/instalments/run.sh
...
assert any('installmentWidgetConfig' in f for f in after), 'Raty config function missing'
print(f'Backend spec verified; {len(after)-len(before)} added, none removed.')
PY
node scripts/check-convex-contract.mjs --strict --spec "$BACKUP/spec-after.json"
