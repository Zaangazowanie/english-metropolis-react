#!/bin/bash
# Bajla connect popup: auto-open only inside the student app, compact design.
# Frontend-only release (index.html + hashed assets); no Convex, no widget.
# Run as root: bash /root/englishmetro/deploy/deploy-bajla-popup-2026-09-11.sh
set -euo pipefail
umask 022
...
BACKUP=/root/backups/englishmetro-bajla-popup-$STAMP
. "$(dirname "$0")/_guard.sh"
guard_clean_prod "$REPO"
cd "$REPO"
exec 9>/root/englishmetro-bajla-popup-deploy.lock
flock -n 9
...
grep -q 'bjp-actions' dist/assets/*.js
...
trap - ERR
push_prod "$REPO"
echo "Bajla popup release published at $REV. Evidence: $BACKUP"
