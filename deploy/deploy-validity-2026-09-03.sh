#!/bin/bash
# Ship package validity (Regulamin § 5 ust. 2, revision 5, 2026-09-03).
#
...
#
# Tested: 77/77 handler tests (/root/em-scheduling-tests/run.sh), convex tsc
# clean, vite build clean, static terms diff limited to the intended lines.
#
# Run as root:  bash /root/englishmetro/deploy/deploy-validity-2026-09-03.sh
set -euo pipefail
...
"$CONVEX" function-spec --prod > "$SPEC_DIR/function-spec-before-validity-$TS.json"
echo "  saved $SPEC_DIR/function-spec-before-validity-$TS.json"
...
"$CONVEX" function-spec --prod > "$SPEC_DIR/function-spec-after-validity-$TS.json"
specguard "$SPEC_DIR/function-spec-before-validity-$TS.json" "$SPEC_DIR/function-spec-after-validity-$TS.json"

echo "== 5/7 SPA call sites vs the NEW prod spec"
cd "$SPA"
node scripts/check-convex-contract.mjs --self-test --spec "$SPEC_DIR/function-spec-after-validity-$TS.json"
node scripts/check-convex-contract.mjs --strict   --spec "$SPEC_DIR/function-spec-after-validity-$TS.json"

echo "== 6/7 site: additive rsync of the prebuilt dist/ with a backup dir"
test -f "$WEB/index.html" && test -d "$WEB/assets"
grep -lq "Ważność: 24 miesiące od zakupu" "$SPA"/dist/assets/index-*.js
...
echo "  convex: redeploy the previous tree; BEFORE spec is $SPEC_DIR/function-spec-before-validity-$TS.json"
echo
