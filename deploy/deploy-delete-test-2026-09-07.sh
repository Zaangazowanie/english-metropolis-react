#!/bin/bash
# Backend-only deploy: adds admin:deleteTestStudents (superadmin, archived
# students only). No schema change, no handler edits, no frontend switch.
set -euo pipefail
umask 022
REPO=/root/englishmetro
STAMP=$(date -u +%Y%m%d-%H%M%S)
BACKUP=/root/backups/englishmetro-delete-test-$STAMP
. "$(dirname "$0")/_guard.sh"
...
assert 'admin.js:deleteTestStudents' in after, 'delete mutation missing from deployed spec'
